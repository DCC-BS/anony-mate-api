...
class DocumentConversionService:
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> httpx.Response:
        ...
        try:
            ...
            logger.exception(
                "Docling API HTTP error", url=url, status_code=e.response.status_code, body=e.response.text
            )
            raise ApiErrorException({
            ...
            logger.exception("Docling connection error", url=url, error=str(e))
            raise ApiErrorException({
                "errorId": DOCUMENT_CONVERSION_ERROR,
                "status": status.HTTP_500_INTERNAL_SERVER_ERROR,
                "debugMessage": f"Docling connection error: {e!s}",
            }) from e
        else:
            return response
    ...
    async def poll_task_status(self, task_id: str) -> None:
        ...
        while True:
            ...
            try:
                ...
                elapsed = time.monotonic() - start_time
                logger.exception("Docling task conversion timed out", task_id=task_id, elapsed=elapsed)
                raise ApiErrorException({
                    "errorId": DOCUMENT_CONVERSION_TIMEOUT,
                    "status": status.HTTP_504_GATEWAY_TIMEOUT,
                    "debugMessage": f"Docling conversion timed out after {elapsed:.1f}s",
